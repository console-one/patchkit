...
export type Paths<T, Prev extends string = ""> = {
  [K in keyof T & (string | number)]: T[K] extends Primitive
    ? `${Prev}${K & string}`
    : `${Prev}${K & string}` | Paths<T[K], `${Prev}${K & string}.`>;
...
export type PatchEvent<T> = {
  [P in Paths<T>]: {
    path: P;
    value:
      | { command: typeof OBJECT_COMMAND.SET; value: PathValue<T, P> }
      | { command: typeof OBJECT_COMMAND.DELETE }
      | { command: typeof OBJECT_COMMAND.DEFAULT; value: PathValue<T, P> };
  };
}[Paths<T>];
...
export type TypedObjectPatch<T> = ObjectPatch & {
  [K in keyof T & string]?:
    | { command: typeof OBJECT_COMMAND.SET; value: T[K] }
    | { command: typeof OBJECT_COMMAND.DELETE }
    | { command: typeof OBJECT_COMMAND.DEFAULT; value: T[K] }
    | { command: typeof OBJECT_COMMAND.CUT; value: string }
    | { command: typeof OBJECT_COMMAND.COPY; value: string }
